    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-200px)]">
        <div className="text-center">
          <AlertCircleIcon className="mx-auto h-10 w-10 text-muted-foreground mb-3" />
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="rounded-xl border bg-card p-6">
            <div className="flex flex-col items-center text-center mb-6">
              <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center text-xl font-bold text-primary mb-3">
                {initials}
              </div>
          ...
          <div className="rounded-xl border bg-card overflow-hidden">
            <div className="flex border-b">
              ...
              >
                ...
                Quotes
                <span className="ml-1 text-xs text-muted-foreground">({quotesTotal})</span>
              </button>
              <button
              ...
              >
                ...
                Shipments
                <span className="ml-1 text-xs text-muted-foreground">({shipmentsTotal})</span>
              </button>
            </div>
            ...
            <div className="divide-y">
              {activeTab === "quotes"
                ? quotes.length === 0
                  ? emptyState(<FileTextIcon className="h-10 w-10" />, "No quotes yet")
                  : quotes.map((q) => (
                      <div key={q.id} className="p-4 hover:bg-muted/30 transition-colors">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{q.goodsType}</p>
                          </div>
                          <Badge variant="outline" className={statusStyles[q.status] || statusStyles.pending}>
                            {q.status.replace("_", " ")}
                          </Badge>
                        </div>
                        <div className="flex gap-6 text-sm text-muted-foreground">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {q.price != null ? q.price.toLocaleString() : "Pending"}
                          </span>
                          <span className="flex items-center gap-1">
                ...
                : shipments.length === 0
                  ? emptyState(<TruckIcon className="h-10 w-10" />, "No shipments yet")
                  : shipments.map((s) => (
                      <div key={s.id} className="p-4 hover:bg-muted/30 transition-colors">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <Badge variant="outline" className={statusStyles[s.status] || statusStyles.pending}>
                            {s.status.replace("_", " ")}
                          </Badge>
                        </div>
                        <div className="flex gap-6 text-sm text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <MapPinIcon className="h-3 w-3" /> {s.origin} → {s.destination}
                          </span>
                          <span className="flex items-center gap-1">
                            <ClockIcon className="h-3 w-3" />{" "}
