...
export default function CarrierAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<CarrierAnalyticsResult>(url)
      .then((res) => {
        if (ignore) return
        setData(res)
        setLoading(false)
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Performance metrics and trends per carrier
          </p>
        </div>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {pct(data.summary.overallOnTimeRate)}
                </div>
                <p className="text-xs text-muted-foreground">
                  Within {slaDays}-day SLA
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {days(data.summary.avgTransitDays)}
                </div>
                <p className="text-xs text-muted-foreground">
                  Across all carriers
                </p>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Highest on-time rate (min 5 delivered)
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-red-600">
                  {data.summary.worstPerformer}
                </div>
                <p className="text-xs text-muted-foreground">
                  Lowest on-time rate (min 5 delivered)
                </p>
              </CardContent>
            </Card>
          </div>
                ...
                <TableBody>
                  {data.carriers.map((c) => (
                    <TableRow
                      ...
                      onClick={() =>
                        setSelectedCarrier(
                          selectedCarrier === c.carrier ? null : c.carrier
                        )
                      }
                    >
                      ...
                      <TableCell className="text-right">
                        <span
                          className={
                            c.exceptionRate > 0.1 ? "text-red-600" : ""
                          }
                        >
              ...
              <CardContent className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={selectedCarrierData.trend}>
                    ...
                    />
                    <Tooltip formatter={(value) => pct(value as number)} />
                    <Line
                      type="monotone"
          ...
          <Card>
            ...
            <CardContent className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={data.carriers.filter((c) => c.exceptionCount > 0)}
                >
                  <CartesianGrid strokeDasharray="3 3" />
