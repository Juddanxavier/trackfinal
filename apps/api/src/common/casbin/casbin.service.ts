...
export class CasbinService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    await this.enforcer.addFunction('globMatch', (pattern: string, text: string) => {
  ...
  private async loadPolicies() {
    ...
    await this.enforcer.addPolicy('staff', '*', 'organisations', 'read', 'allow');
    ...
    await this.enforcer.addPolicy('customer', '*', 'shipments', 'read:own', 'allow');
    await this.enforcer.addPolicy('customer', '*', 'shipments', 'write:own', 'allow');
    await this.enforcer.addPolicy('customer', '*', 'quotes', 'read:own', 'allow');
    await this.enforcer.addPolicy('customer', '*', 'quotes', 'write:own', 'allow');
    await this.enforcer.addPolicy('customer', '*', 'users', 'read:own', 'allow');

    this.logger.log('Policies loaded');
  }

  async can(sub: string, obj: string, act: string, org?: string): Promise<boolean> {
    const orgId = org || '';
    try {
  ...
  private async logAudit(userId: string, orgId: string, action: string, object: string, result: boolean) {
    try {
      const roles = (await this.enforcer.getRolesForUser(userId)).slice(0, 5) as string[];
      this.auditLogs.push({
        timestamp: new Date(),
        userId,
        orgId,
  ...
  async getAuditLogs(filters?: { userId?: string; orgId?: string; limit?: number }): Promise<AuditLogEntry[]> {
  ...
  async canMany(sub: string, permissions: Array<{ obj: string; act: string }>, org?: string): Promise<boolean[]> {
    return Promise.all(permissions.map(p => this.can(sub, p.obj, p.act, org)));
  }
  ...
  async getPermissionsForUser(userId: string): Promise<Array<{ org: string; obj: string; act: string }>> {
  ...
  async syncUserRole(userId: string, role: string, orgId?: string): Promise<void> {
    await this.enforcer.deleteUser(userId);
    if (role) {
      await this.enforcer.addRoleForUser(userId, role);
      this.logger.log(`Synced user '${userId}' to role '${role}' org '${orgId || '*'}'`);
    }
  }
  ...
  async setUserBranchPermissions(userId: string, branchId: string, role: string): Promise<void> {
    ...
    await this.enforcer.addPolicy('staff', branchId, 'reports', 'read', 'allow');
    await this.enforcer.addPolicy('staff', branchId, 'notifications', '*', 'allow');
    await this.enforcer.addPolicy('staff', branchId, 'organisations', 'read', 'allow');
    this.logger.log(`Added branch permissions for staff user '${userId}' on branch '${branchId}'`);
  }

  async clearUserBranchPermissions(userId: string, branchId: string): Promise<void> {
    await this.enforcer.removePolicy('staff', branchId, 'shipments', '*', 'allow');
    await this.enforcer.removePolicy('staff', branchId, 'quotes', '*', 'allow');
    await this.enforcer.removePolicy('staff', branchId, 'tracking', '*', 'allow');
    await this.enforcer.removePolicy('staff', branchId, 'reports', 'read', 'allow');
    await this.enforcer.removePolicy('staff', branchId, 'notifications', '*', 'allow');
    await this.enforcer.removePolicy('staff', branchId, 'organisations', 'read', 'allow');
    this.logger.log(`Cleared branch permissions for user '${userId}' on branch '${branchId}'`);
  }

  async clearAllBranchPermissions(branchId: string): Promise<void> {
    const objects = ['shipments', 'quotes', 'tracking', 'reports', 'notifications', 'organisations'];
    const actions = ['*', 'read'];
    for (const obj of objects) {
      for (const act of actions) {
        await this.enforcer.removePolicy('staff', branchId, obj, act, 'allow').catch(() => {});
      }
    }
  }

  async canAccessBranch(userId: string, branchId: string, object: string, action: string): Promise<boolean> {
  ...
  async updateUserPermissions(userId: string, orgId: string, permissions: Array<{ obj: string; act: string }>): Promise<void> {
  ...
  async addPolicy(sub: string, obj: string, act: string, org?: string): Promise<void> {
    await this.enforcer.addPolicy(sub, org || '', obj, act, 'allow');
  }

  async removePolicy(sub: string, obj: string, act: string, org?: string): Promise<void> {
