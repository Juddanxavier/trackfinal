...
import {
  ArrowRight, Box, Ruler, HeadphonesIcon,
  Package, RefreshCw
} from 'lucide-react';
...
export default function VolumetricCalculatorPage() {
  ...
  const actualOk = aw > 0;
  const chargeableWeight = volumetricWeight !== null && actualOk
        ...
        <div className='max-w-7xl mx-auto px-6 lg:px-8'>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            <p className='text-lg sm:text-xl text-zinc-700 max-w-2xl mt-6 leading-relaxed'>
              Calculate volumetric (dimensional) weight for your shipments. When dimensional weight exceeds actual weight, the charge is based on the IATA volumetric standard.
            </p>
          </motion.div>
        </div>
            ...
            >
              ...
              <div className='bg-zinc-50 border border-zinc-100 p-8 lg:p-10'>
                ...
                <div className='flex gap-2 mb-8 bg-white p-1 border border-zinc-200 rounded-lg w-fit'>
                  {(Object.entries(units) as [UnitSystem, typeof units[UnitSystem]][]).map(([key, u]) => (
                    <button
                      key={key}
                ...
                <div className='mb-8'>
                  ...
                  <p className='text-xs text-zinc-500 mt-1.5'>
                    Optional. Enter the actual weight to determine which is greater for billing.
                  </p>
                </div>
              ...
              <div className='bg-zinc-50 border border-zinc-100 p-8 lg:p-10 sticky top-28'>
                ...
                {!isValid ? (
                  <div className='text-center py-10'>
                    ...
                    <p className='text-sm text-zinc-500'>Enter dimensions to calculate volumetric weight</p>
                  </div>
                ) : (
                  <div className='space-y-6'>
                    {/* Volumetric Weight */}
                    <div className='bg-white border border-zinc-200 rounded-lg p-5 text-center'>
                      <p className='text-xs text-zinc-500 mb-1'>Volumetric Weight</p>
                      <p className='text-2xl font-bold text-zinc-900 font-heading'>
                        {volumetricWeight!.toFixed(2)} <span className='text-sm font-normal text-zinc-500'>{weightUnit}</span>
                      </p>
                    ...
                    {/* Actual Weight */}
                    <div className='bg-white border border-zinc-200 rounded-lg p-5 text-center'>
                      <p className='text-xs text-zinc-500 mb-1'>Actual Weight</p>
                      <p className='text-2xl font-bold text-zinc-900 font-heading'>
                        {actualOk ? `${aw.toFixed(2)}` : '\u2014'} <span className='text-sm font-normal text-zinc-500'>{actualOk ? weightUnit : ''}</span>
                      </p>
                    </div>

                    {/* Chargeable Weight */}
                    <div className={`rounded-lg p-5 text-center border-2 ${
                      chargeableWeight !== null && actualOk && chargeableWeight > aw
                        ? 'bg-primary/5 border-primary/20'
                        : 'bg-white border-zinc-200'
                    }`}>
                      <p className='text-xs text-zinc-500 mb-1'>Chargeable Weight</p>
                      <p className='text-2xl font-bold text-zinc-900 font-heading'>
                        {chargeableWeight!.toFixed(2)} <span className='text-sm font-normal text-zinc-500'>{weightUnit}</span>
                      </p>
                    ...
                    <div className='p-4 bg-zinc-100 rounded-lg'>
                      <p className='text-xs text-zinc-600 leading-relaxed'>
                        <strong>Note:</strong> If the volumetric weight exceeds the actual weight, the shipment is charged
                        based on the dimensional weight per IATA standards. This ensures the package density is accounted for.
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className='max-w-7xl mx-auto px-6 lg:px-8'>
          ...
          <div className='grid md:grid-cols-3 gap-8'>
            {[
              ...
              >
                <span className='text-4xl font-bold text-primary/10 font-heading block mb-4'>{item.step}</span>
                <h3 className='text-base font-semibold text-zinc-900 mb-3'>{item.title}</h3>
                <p className='text-base text-zinc-700 leading-relaxed'>{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className='max-w-7xl mx-auto px-6 lg:px-8 text-center'>
          <motion.div {...fadeIn}>
            ...
            <p className='text-zinc-700 max-w-lg mx-auto mb-10 text-lg'>
              Get instant quotes, calculate volumetric weight, and track your shipments — all in one place.
            </p>
