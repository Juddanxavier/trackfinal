...
function mapApiQuoteToQuote(apiQuote: ApiQuote): Quote {
  return {
    ...
    weight:
      typeof apiQuote.weight === "number"
        ? `${apiQuote.weight}`
        : apiQuote.weight.replace(/\s*kg$/i, ""),
    serviceType:
      apiQuote.goodsType.charAt(0).toUpperCase() + apiQuote.goodsType.slice(1),
        ...
        <div className="p-4">
          ...
          {quote.price && quote.status === "quoted" && (
            <motion.div
              ...
              className="mt-3 p-4 text-white rounded-lg overflow-hidden relative"
              style={{ backgroundColor: "var(--primary)" }}
            >
          ...
          {quote.price && quote.status !== "quoted" && (
            <div className="mt-3 p-3 bg-zinc-50 rounded-lg border border-zinc-100">
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${quote.status === "accepted" ? "bg-emerald-100" : "bg-red-100"}`}
                >
                  {quote.status === "accepted" && (
          ...
          {quote.status === "quoted" && (
            <div className="border-t border-zinc-100 pt-3 mt-3">
              <p className="text-xs text-zinc-500 mb-3 text-center">
                Ready to confirm? Accept or decline this quote.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => onAcceptQuote?.(quote.id)}
                  disabled={isAccepting === quote.id}
                  className="flex-1 px-4 py-2.5 text-white text-sm font-semibold rounded-lg transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20"
                  style={{ backgroundColor: "var(--primary)" }}
                >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600/80">
                      You can request a new quote anytime
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-3.5 bg-white rounded-lg border border-zinc-200/70 hover:border-primary/30 hover:shadow-sm transition-all">
        ...
        {quote.price && (
          <span
            className="font-bold text-sm shrink-0 min-w-[70px] text-right"
...
function StatsCard({
  ...
  stats: { total: number; pending: number; quoted: number; accepted: number };
}) {
  const total = stats.total;
  const acceptedRate =
    total > 0 ? Math.round((stats.accepted / total) * 100) : 0;
      ...
      >
        <div className="mb-3">
          ...
          <p className="text-zinc-500 text-xs mt-0.5">
            Get competitive rates + free packing with every shipment
          </p>
        </div>
        <button
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-zinc-500 text-center mb-5 max-w-sm">
        Request your first quote to get competitive rates for your international
        shipments
      </p>
      <button
        onClick={onOpenDialog}
        className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium text-white shadow-lg shadow-primary/20 cursor-pointer"
        style={{ backgroundColor: "var(--primary)" }}
      >
        <Plus className="w-4 h-4" />
        Request Quote
              ...
              <div className="lg:col-span-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="h-44 bg-zinc-100 rounded-xl animate-pulse"
                    />
                  ))}
                </div>
              </div>
        ...
        <main className="max-w-6xl mx-auto px-4 pt-24 pb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white shadow-lg shadow-primary/20 cursor-pointer"
              style={{ backgroundColor: "var(--primary)" }}
            >
              <Plus className="w-4 h-4" />
              New Quote
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4">
              <Sidebar
                stats={stats}
                onOpenDialog={() => setShowQuoteDialog(true)}
              />
            </div>
          </div>
        </main>
