...
import {
  Search, ArrowRight, MapPin, Clock, Truck, Package,
  CheckCircle, XCircle, AlertTriangle, ChevronRight,
  HelpCircle, Phone, Mail, Globe, ShieldCheck, Headphones,
} from 'lucide-react';
...
const statusConfig: Record<string, { label: string; icon: any; color: string; gradient: string }> = {
  pending:    { label: 'Pending',     icon: Clock,        color: 'text-amber-600', gradient: 'from-amber-50 via-amber-50/80 to-amber-100/30' },
  in_transit: { label: 'In Transit',  icon: Truck,        color: 'text-blue-600',  gradient: 'from-blue-50 via-blue-50/80 to-blue-100/30' },
  delivered:  { label: 'Delivered',   icon: CheckCircle,  color: 'text-emerald-600', gradient: 'from-emerald-50 via-emerald-50/80 to-emerald-100/30' },
  cancelled:  { label: 'Cancelled',   icon: XCircle,      color: 'text-zinc-700',  gradient: 'from-zinc-50 via-zinc-50/80 to-zinc-100/30' },
  exception:  { label: 'Exception',   icon: AlertTriangle, color: 'text-red-600',  gradient: 'from-red-50 via-red-50/80 to-red-100/30' },
};
...
function TrackContent() {
  ...
  const fetchTracking = async (trackingCode: string) => {
    ...
    setResult(null);
    try {
      const res = await fetch(`${API_URL}/shipments/public/track/${trackingCode}`);
      if (!res.ok) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmed) return;
    setSearched(true);
    router.push(`/track?code=${encodeURIComponent(trimmed)}`, { scroll: false });
  };
    ...
    <>
      ...
      <main className='min-h-screen bg-zinc-50'>
            ...
            <div className='grid lg:grid-cols-2 gap-12 items-center'>
              ...
              >
                <div className='flex items-center gap-3 mb-5'>
                  ...
                  <span className='text-primary text-sm font-semibold tracking-[0.2em] uppercase'>Track a Shipment</span>
                </div>
                ...
                <div className='bg-white rounded-2xl p-6 md:p-8 border border-zinc-100 hover:border-primary/20 transition-all duration-300'>
                  <form onSubmit={handleSubmit} className='space-y-3.5'>
                    ...
                    <button
                      ...
                      style={{ background: !code.trim() || loading ? '#e63329' : 'linear-gradient(135deg, #e63329 0%, #cc2b22 100%)' }}
                    >
        ...
        <section className='pb-16 lg:pb-24 px-6 lg:px-8 bg-zinc-50'>
          <div className={result && !loading ? 'max-w-7xl mx-auto' : 'max-w-3xl mx-auto'}>
            {!searched && !loading && (
              <motion.div
                initial={{ opacity: 0, y: 16 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <h2 className='text-lg font-semibold text-zinc-900 mb-2'>Awaiting input</h2>
                <p className='text-sm text-zinc-700 max-w-xs mx-auto leading-relaxed'>
                  Enter a tracking number above to see real-time shipment status, timeline, and delivery details.
                </p>
            ...
            {error && !loading && (
              ...
              >
                ...
                <h2 className='text-lg font-semibold text-zinc-900 mb-1'>Shipment not found</h2>
                <p className='text-sm text-zinc-700 mb-6 max-w-xs mx-auto leading-relaxed'>
                  No shipment found with that ID. Please check the number and try again.
                </p>
                <button
                  onClick={() => { setError(''); setCode(''); setSearched(false); }}
            ...
            {result && !loading && (
              <div className='grid lg:grid-cols-3 gap-8'>
                <div className='lg:col-span-2 space-y-5'>
                  ...
                  >
                    <div className={`bg-gradient-to-r ${statusConfig[result.status]?.gradient || 'from-zinc-50'} px-5 md:px-6 py-4 flex items-center justify-between`}>
                      <div className='flex items-center gap-3'>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-white shadow-sm ${statusConfig[result.status]?.color || 'text-zinc-700'}`}>
                          {React.createElement(statusConfig[result.status]?.icon || Package, { className: 'w-5 h-5' })}
                        </div>
                        <div>
                          <p className={`text-sm font-semibold ${statusConfig[result.status]?.color || 'text-zinc-700'}`}>
                            {statusConfig[result.status]?.label || 'Unknown'}
                          </p>
                          <p className='text-sm text-zinc-400 mt-0.5 font-mono tracking-wider'>{result.trackingNumber}</p>
                        </div>
                      </div>
                    ...
                    <div className='px-5 md:px-6 py-6'>
                      <div className='flex items-center justify-between gap-4'>
                        <div className='text-center flex-1'>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-2'>Origin</p>
                          <div className='flex items-center justify-center gap-2'>
                            {countryToFlag[result.originCountry] && (
                              <ReactCountryFlag
                                countryCode={countryToFlag[result.originCountry]}
                                svg
                                style={{ width: '1.25em', height: '1.25em', borderRadius: '2px' }}
                              />
                            )}
                            <p className='text-sm font-semibold text-zinc-900'>{result.originCountry}</p>
                          </div>
                        </div>
                        ...
                        <div className='text-center flex-1'>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-2'>Destination</p>
                          <div className='flex items-center justify-center gap-2'>
                            {countryToFlag[result.destinationCountry] && (
                              <ReactCountryFlag
                                countryCode={countryToFlag[result.destinationCountry]}
                                svg
                                style={{ width: '1.25em', height: '1.25em', borderRadius: '2px' }}
                              />
                            )}
                            <p className='text-sm font-semibold text-zinc-900'>{result.destinationCountry}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    <div className='px-5 md:px-6 py-4 border-b border-zinc-100'>
                      <div className='flex items-center gap-3'>
                        ...
                        <span className='text-sm font-semibold text-zinc-900'>Tracking Timeline</span>
                        {result.events.length > 0 && (
                          <span className='text-xs text-zinc-400'>({result.events.length} events)</span>
                        )}
                      </div>
                    </div>
                    <div className='px-5 md:px-6 py-5'>
                      {result.events.length === 0 ? (
                        <div className='text-center py-10'>
                          ...
                          <p className='text-sm text-zinc-700 font-medium'>No tracking updates yet</p>
                          <p className='text-xs text-zinc-400 mt-1'>Updates will appear once the shipment starts moving</p>
                        </div>
                      ) : (
                        <div className='relative'>
                          <div className='absolute left-[17px] top-2 bottom-2 w-0.5 bg-gradient-to-b from-primary via-zinc-200 to-zinc-200' aria-hidden='true' />
                                ...
                                >
                                  ...
                                  <div className='flex-1 min-w-0 pt-1.5'>
                                    <div className='flex items-center gap-2 flex-wrap'>
                                      <p className={`text-sm font-semibold ${isLatest ? 'text-zinc-900' : 'text-zinc-700'}`}>
                                        {event.status}
                                      </p>
                                      <span className={`text-sm px-2 py-0.5 rounded font-medium ${
                                        isLatest ? 'bg-primary/10 text-primary' : 'bg-zinc-100 text-zinc-700'
                                      }`}>
                                        {date}
                                      </span>
                                    </div>
                                    <p className='text-sm text-zinc-700 mt-0.5'>{event.description}</p>
                  ...
                  >
                    ...
                    <h3 className='text-sm font-semibold text-zinc-900 mb-1'>Need help with your shipment?</h3>
                    <p className='text-sm text-zinc-700 max-w-sm mx-auto leading-relaxed mb-4'>
                      Our support team is available 24/7 to assist with any delivery questions.
                    </p>
                ...
                <aside className='lg:col-span-1 space-y-4'>
                  ...
                  >
                    <div className='px-5 md:px-6 py-4 border-b border-zinc-100'>
                      <div className='flex items-center gap-3'>
                        ...
                        <span className='text-sm font-semibold text-zinc-900'>Shipment Details</span>
                      </div>
                    </div>
                    <div className='px-5 md:px-6 py-5'>
                      <div className='grid grid-cols-2 gap-y-5 gap-x-6'>
                        <div>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Recipient</p>
                          <p className='text-sm text-zinc-900 font-medium'>{result.recipientName}</p>
                        </div>
                        <div>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Carrier</p>
                          <p className='text-sm text-zinc-900 font-medium'>{result.carrierName || result.carrierCode || '\u2014'}</p>
                        </div>
                        <div>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Created</p>
                          <p className='text-sm text-zinc-900 font-medium'>{formatDate(result.createdAt).date}</p>
                        </div>
                        <div>
                          <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Status</p>
                          <p className={`text-sm font-medium ${statusConfig[result.status]?.color || 'text-zinc-900'}`}>
                            {statusConfig[result.status]?.label || 'Unknown'}
                          </p>
                        </div>
                        {result.deliveredAt && (
                          <>
                            <div>
                              <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Delivered</p>
                              <p className='text-sm font-medium text-zinc-900'>{formatDate(result.deliveredAt).date}</p>
                            </div>
                            <div>
                                <p className='text-sm text-zinc-500 uppercase tracking-wider mb-1.5'>Last Location</p>
                                <p className='text-sm text-zinc-900 font-medium'>{result.events[0].location}</p>
                            </div>
                          </>
                        )}
                      </div>
                  ...
                  <div className='flex items-center gap-2 mb-1'>
                    ...
                    <span className='text-primary text-sm font-semibold tracking-[0.2em] uppercase'>Quick Links</span>
                  </div>
                  {[
                    { icon: HelpCircle, title: 'Find tracking number', desc: 'Check your confirmation email or SMS for the 12-digit GT code.' },
                    { icon: Clock, title: 'Delivery times', desc: 'Express 2-5 days, Standard 7-15 days to most countries.' },
                    { icon: Globe, title: 'Global coverage', desc: '200+ countries and territories with real-time tracking.' },
                  ].map((item, i) => (
                    <motion.div
                      key={i}
                    ...
                    >
                      <div className='flex items-start gap-3'>
                        ...
                        <div className='min-w-0'>
                          ...
                          <p className='text-[11px] text-zinc-700 mt-0.5 leading-relaxed'>{item.desc}</p>
                        </div>
                      </div>
                    </motion.div>
                  ))}
                  ...
                  >
                    <h4 className='text-sm font-semibold text-zinc-900 mb-2'>Contact support</h4>
                    <p className='text-[11px] text-zinc-700 leading-relaxed mb-3'>
                      Available 24/7 for tracking issues and delivery questions.
                    </p>
                    <div className='space-y-2'>
                      <a href='mailto:support@gajantraders.com' className='flex items-center gap-2 text-[11px] text-zinc-700 hover:text-primary transition-colors'>
                        <Mail className='w-3 h-3' />
                        support@gajantraders.com
                      </a>
                      <a href='tel:+1234567890' className='flex items-center gap-2 text-[11px] text-zinc-700 hover:text-primary transition-colors'>
            ...
            <div className='max-w-7xl mx-auto'>
              ...
              >
                <div className='flex items-center gap-3 mb-8'>
                  ...
                  <span className='text-primary text-sm font-semibold tracking-[0.2em] uppercase'>Quick Guide</span>
                </div>
              </motion.div>
              <div className='grid md:grid-cols-3 gap-4'>
                {[
                  {
                    icon: HelpCircle, title: 'Find your tracking number',
                    desc: 'Located in your shipment confirmation email or SMS. It is a 12-digit code starting with "GT".',
                  },
                  {
                    icon: Clock, title: 'Delivery times',
                  ...
                  >
                    ...
                    <p className='text-sm text-zinc-700 mt-1.5 leading-relaxed'>{item.desc}</p>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <div className='max-w-3xl mx-auto'>
            ...
            >
              <div className='flex items-center gap-3 mb-5 justify-center'>
                <HelpCircle className='w-5 h-5 text-primary' />
                <span className='text-primary text-sm font-semibold tracking-[0.2em] uppercase'>Help Center</span>
              </div>
                ...
                >
                  <details className='group bg-zinc-50 rounded-xl overflow-hidden border border-zinc-100 hover:border-zinc-200 transition-all duration-300'>
                    ...
                    <div className='px-5 md:px-6 pb-4 md:pb-5'>
                      <p className='text-sm text-zinc-700 leading-relaxed max-w-prose'>{item.a}</p>
                    </div>
                  </details>
                </motion.div>
              ))}
