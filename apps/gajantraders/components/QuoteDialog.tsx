...
const destinationCountries = [
  'Australia', 'Bangladesh', 'Bhutan', 'Canada', 'China', 'Dubai (UAE)', 'France',
  'Germany', 'Hong Kong', 'Indonesia', 'Italy', 'Japan', 'Kenya', 'Malaysia',
  'Nepal', 'Netherlands', 'New Zealand', 'Pakistan', 'Philippines', 'Singapore',
  'South Africa', 'South Korea', 'Spain', 'Sri Lanka', 'Thailand', 'UK', 'USA', 'Vietnam',
];

function Combobox({ value, onChange, options, placeholder, error }: { value: string; onChange: (val: string) => void; options: string[]; placeholder: string; error?: string }) {
...
export default function QuoteDialog({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  const { user } = useAuth();
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Goods Type</label>
              <select
              ...
              >
                {goodsTypes.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
