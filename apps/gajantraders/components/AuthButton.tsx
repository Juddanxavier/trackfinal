...
export default function AuthButton({
  ...
  const textColorMuted =
    scrolled || !isHome ? "text-gray-500" : "text-white/60";
  const borderColor =
    scrolled || !isHome ? "border-gray-200" : "border-white/20";
  const hoverBorderColor =
    scrolled || !isHome ? "hover:border-gray-300" : "hover:border-white/40";
    ...
    return (
      <div className="relative z-50">
        ...
        >
          ...
          <span
            className={`text-sm font-medium hidden md:block transition-colors duration-300 ${textColor}`}
          >
            {user.name.split(" ")[0]}
          </span>
          <ChevronDown
            className={`w-4 h-4 transition-all duration-300 ${dropdownOpen ? "rotate-180" : ""} ${textColorMuted}`}
          />
        </button>
